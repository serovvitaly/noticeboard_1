<div class="toolbar">
  <div class="btn-group">
    <button class="btn btn-small" title="Показать/скрыть картинки" data-toggle="button"><i class="icon-picture"></i></button>
    <button class="btn btn-small"><i class="icon-search"></i></button>
    <button class="btn btn-small"><i class="icon-search"></i></button>
  </div>
</div>

<div class="row announcements-list">
  <div class="announcements-item-6"><div class="wrapper">
      <img data-src="holder.js/140x140" class="img-polaroid" alt="140x140" style="width: 81px; height: 81px;" src="data:image/png;base64,">
      
        <strong>Заголовок объявления о продаже чего то</strong>
      
  </div></div>
  <div class="announcements-item-6">...</div>
  <div class="announcements-item-6">...</div>
  <div class="announcements-item-6">...</div>
  <div class="announcements-item-6">...</div>
  <div class="announcements-item-6">...</div>
  <div class="announcements-item-6">...</div>
  <div class="announcements-item-6">...</div>
  <div class="announcements-item-6">...</div>
</div>

<div class="announcements-list-pagination">
  PAGINATION
</div>